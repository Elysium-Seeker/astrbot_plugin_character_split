...
为保证每次推送都被 AstrBot 识别为新版本，请使用仓库根目录脚本：

```bash
python update.py --level patch --note "本次更新内容"
```

脚本会自动执行：
- 更新 `metadata.yaml` 的 `version`。
- 同步 `main.py` 中 `@register(...)` 的版本号。
- 追加 `CHANGELOG.md` 条目并同步写入“最近更新”段落。
- 自动 `git add/commit/tag/push`。

常用示例：

```bash
# 小版本修复
python update.py --level patch --note "修复会话模式切换边界问题"

# 功能更新（次版本）
python update.py --level minor --note "新增自动记忆压缩策略" --note "新增配置项说明"
```
