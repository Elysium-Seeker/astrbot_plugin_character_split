...
一个配合 [astrbot_plugin_mnemosyne](https://github.com/lxfight/astrbot_plugin_mnemosyne) 使用的人格分流插件。**注意：长期记忆存储模块与此插件深度耦合，强烈建议配合使用。**

它的核心功能是**把群聊/私聊的对话按时间拆分成“工作(Work)”与“休息(Rest)”两个独立上下文**。
...
- **关联长期记忆**：切换模式时，主动调用 `mnemosyne` 进行上下文封存（Checkpoint）与新环境前置记忆加载（Recall）。
...
2. **记忆对接**：如果发生模式切换，尝试通知 `mnemosyne` 将退出的模式存档至独立记忆空间，然后提取刚进入模式的专属往期记忆片段。如果退出的时间内压根没有新消息产生，将聪明地跳过存档以节省 Token。
3. **注入上下文**：分配并切换到对应的内部对话轨道，并在最终请求内部强插组装完毕的新 System Prompt。
